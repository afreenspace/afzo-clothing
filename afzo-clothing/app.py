...
def products():
    ...
    if q:
        filtered = [p for p in filtered if q in p['name'].lower() or q in p['desc'].lower() or q in p['category'].lower()]
    return render_template('products.html', products=filtered, active=cat, query=q, user=session.get('user'))
